...
(async function () {
  ...
  // Listen for tab activation/deactivation messages from the background service worker
  if (typeof chrome !== "undefined" && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((message) => {
      if (message?.type === "tabDeactivated") {
        isTabActive = false;
        // Disconnect observer to stop processing mutations on background tabs
        if (currentObserver) {
          currentObserver.disconnect();
          currentObserver = null;
        }
        ...
        isTabActive = true;
        // If setup never completed (wrapper wasn't found while tab was in background), retry
        if (!setupComplete) {
          attempt = 0;
          trySetup();
        } else {
          // Re-scan and reconnect observer now that tab is active again
          applyBlockListAndFilter();
        }
      }
    });
